...
for(i in 1:length(n1)){
  eqres[i]<-mean(replicate(10000, simT(n1[i],n1[i],0,0,1,1))<.05) # equal var
  uneqres[i]<-mean(replicate(10000, simT(n1[i],n1[i],0,0,1,2))<.05) # unequal var
  uneqsampres[i]<-mean(replicate(10000, simT(n1[i],n2[i],0,0,1,2))<.05) # unequal var and sample size
  hiuneqsampres[i]<-mean(replicate(10000, simT(n1[i],n3[i],0,0,1,2))<=.05) # unequal var and sample size
}

simdat<-tibble::tibble(
  sizenone=rep(seq(30,120,10),4),
  sim=c(rep("Equal var.\nequaln",length(n1)),
        rep("Unequal var.\nequal n",length(n1)),
        rep("Unequal var.\ndiff. n1 - n2 = 10",length(n1)),
        rep("Unequal var.\ndiff. n1 - n2 = 20",length(n1))),
