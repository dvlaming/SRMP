...
plot<-ggplot() +
  ...
  geom_segment(aes(x = 3, y = 2, xend = 7, yend = 2), linewidth = 1.5,
               arrow = arrow(length = unit(.5,"cm"))) +
  geom_text(aes(x = 2, y = 2 , label = "IV"), colour = "grey") +
  geom_text(aes(x = 8, y = 2 , label = "DV"), colour = "grey") +
  geom_text(aes(x = 5, y = 6 , label = "Mediator"), colour = "grey") +
  geom_text(aes(x = 5, y = 2.25 , label = "c'")) +
  geom_text(aes(x = 3.25, y = 4 , label = "a"), angle = atan2(2,1) * (180 / pi)) +
  geom_text(aes(x = 6.75, y = 4 , label = "b"), angle = 360 - atan2(2,1) * (180 / pi)) +
  geom_text(aes(x = 5, y = 1 , label = "Indirect effect: [CIlo, CIhi]"))
